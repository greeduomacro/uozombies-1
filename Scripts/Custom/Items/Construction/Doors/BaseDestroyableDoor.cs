...
{
    public abstract class BaseDestroyableDoor : DestroyableItem, ILockable, ITelekinesisable
    {
        private bool m_Open, m_Locked;
        private int m_OpenedID, m_OpenedSound;
        private int m_ClosedID, m_ClosedSound;
        private Point3D m_Offset;
        private BaseDestroyableDoor m_Link;
        ...
        {
            BaseDestroyableDoor door = targeted as BaseDestroyableDoor;
        ...
        {
            BaseDestroyableDoor first = (BaseDestroyableDoor)state;
            BaseDestroyableDoor second = targeted as BaseDestroyableDoor;
        ...
        {
            e.Mobile.BeginTarget(-1, false, TargetFlags.None, new TargetStateCallback(ChainLink_OnTarget), new List<BaseDestroyableDoor>());
        ...
        private static void ChainLink_OnTarget(Mobile from, object targeted, object state)
        {
            ...
            {
                List<BaseDestroyableDoor> list = (List<BaseDestroyableDoor>)state;
        ...
        private static void EventSink_OpenDoorMacroUsed(OpenDoorMacroEventArgs args)
        {
            ...
            if (m.Map != null)
            {
                ...
                {
                    if (item.Location.X == x && item.Location.Y == y && (item.Z + item.ItemData.Height) > m.Z && (m.Z + 16) > item.Z && item is BaseDestroyableDoor && m.CanSee(item) && m.InLOS(item))
                    {
                        if (m.CheckAlive())
                        {
        ...
        {
            private BaseDestroyableDoor m_Door;

            public InternalTimer(BaseDestroyableDoor door)
        ...
        private bool CheckFit(Map map, Point3D p, int height)
        {
            ...
            for (int i = 0; i < items.Count; ++i)
            {
                ...
                if (item.ItemID < 0x4000 && item.AtWorldPoint(x, y) && !(item is BaseDestroyableDoor))
                {
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public BaseDestroyableDoor Link
        {
            get
            {
        ...
        public List<BaseDestroyableDoor> GetChain()
        {
            List<BaseDestroyableDoor> list = new List<BaseDestroyableDoor>();
            BaseDestroyableDoor c = this;
        ...
        public bool IsFreeToClose()
        {
            ...
            List<BaseDestroyableDoor> list = GetChain();
        ...
        public virtual void Use(Mobile from)
        {
            ...
            else
            {
                ...
                BaseDestroyableDoor link = this.Link;
        ...
        {
            IDStart = m_OpenedID;
            IDHalfHits = m_OpenedID;
        }

        public virtual void OnClosed(Mobile from)
        {
            IDStart = m_ClosedID;
            IDHalfHits = m_ClosedID;
        }
        ...
        public BaseDestroyableDoor(int closedID, int openedID, int openedSound, int closedSound, Point3D offset)
            : base(closedID, closedID, 3117)
        {
            m_OpenedID = openedID;
            m_ClosedID = closedID;
                ...
                case 0:
                    {
                        ...
                        m_Link = reader.ReadItem() as BaseDestroyableDoor;
